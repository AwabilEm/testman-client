...
export class ProfilePage extends BasePage {
  ...
  async completeProfile() {
    ...
    await this.page.locator('div').filter({ hasText: /^CountrySelect CountryCanadaMexicoUnited States$/ }).getByRole('combobox').selectOption('United States');
    await this.page.getByRole('combobox').nth(2).selectOption('Alabama');
    await this.page.locator('man-input').filter({ hasText: 'City *' }).getByRole('textbox').fill('Accra');
    await this.page.locator('man-input').filter({ hasText: 'Zip Code *' }).getByRole('textbox').fill('11551');
